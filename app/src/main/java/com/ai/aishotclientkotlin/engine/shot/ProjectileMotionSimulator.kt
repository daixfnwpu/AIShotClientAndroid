...
object ProjectileMotionSimulator {
    ...
    fun calculatePointsOnShotHead(
        eyeSlibber: Double,
        yY0Diff: DoubleArray,
        distanceValues: DoubleArray
    ): DoubleArray {
    ...
    suspend fun calculateTrajectory(shotCause: ShotCauseState): List<Position> {
        ...
        val yEndPositionUP =
            targetRealPos.second + abs(targetRealPos.second) * abs(targetRealPos.second) / 100
        val xEndPositionUP = targetRealPos.first * 1.3
        val yEndPositionDown =
            targetRealPos.second - abs(targetRealPos.second) * abs(targetRealPos.second) / 100
...
//       val ay = if (vy > 0) -G - dragForce(vy, A) / m else -G + dragForce(vy, A) / m
    ...
    fun transformPostionsToMotion(
        positions: List<Position>,
        eyeSlibber: Double,
        thetaRad: Double
    ): List<ProjectileMotionData> {
